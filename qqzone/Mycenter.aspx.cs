...
public partial class Mycenter : System.Web.UI.Page
{
    ...
    protected void btnCheck_Click(object sender, EventArgs e)
    {
        ...
        int notice = 0; //判断单选框是否被选中标识，选中为1，不被选中为0
        foreach (RepeaterItem item in this.rptLable.Items)  //遍历repeater标签列表
        {
            RadioButton rbLable = item.FindControl("rbLable") as RadioButton;
            //string rbid = (rptLable.Items[i].FindControl("rbLable") as HtmlInputRadioButton).ID;  //获取当前行的RadioButton的id
            //HtmlInputRadioButton rbLable = rptLable.Items[i].FindControl("rbid") as HtmlInputRadioButton;  //获取当前行的RadioButton
            if (rbLable.Checked == true)  //若单选框被选中
            {
                int i = item.ItemIndex;  //获取当前行的id
                string lablesql = "select * from Lable where userid='" + user.Userid + "' and lablename in (select distinct lablename from Lable where userid='" + user.Userid + "')";
                DataTable labledt = DataClass.DataT(lablesql);  //获得数据表
                string name= labledt.Rows[i][3].ToString();  //获得标签名
                string selectsql = "select friendid from Lable where userid='" + user.Userid + "' and lablename='" + name + "'";  //查询当前用户该标签中的所有好友id
                DataTable dt = DataClass.DataT(selectsql);  //获得数据表
                for(int j=0;j<dt.Rows.Count;j++)
                {
                    int friendid = Convert.ToInt32(dt.Rows[j][0].ToString());  //获取第j+1位好友的id
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        lbLablename.Visible = true;
        txtLabel.Visible = true;
        btnSaveLable.Visible = true;
        btnReturn.Visible = true;
    }

    protected void btnSaveLable_Click(object sender, EventArgs e)
    {
        Users user = (Users)Session["user"];
        string labelname = txtLabel.Text;
        string checksql = "select * from Lable where lablename='" + labelname + "' and userid='" + user.Userid + "'";  //查询当前标签名是否已存在
        int result = DataClass.Select(checksql);
        if (result == 1)
            Response.Write("<script>alert('标签名已存在！')</script>");
        else
        {
            ...
            else
            {
                foreach (RepeaterItem item in this.rptList.Items)  //遍历repeater好友列表
                {
                    ...
                    if (cbFriend.Checked == true)  //当对应复选框被选中时
                    {
                        ...
                        DataTable dt = DataClass.DataT(selectsql);  //获得数据表
                        int friendid = Convert.ToInt32(dt.Rows[i][0].ToString());  //获取被选中的好友id
                        string insertsql = "insert into Lable values('"+user.Userid+"','" + friendid + "',N'" + labelname + "')";
                        DataClass.Save(insertsql);
                        Response.Write("<script>alert('保存成功！');location='Mycenter.aspx'</script>");
                    }
                }
            }
        }
    ...
    protected void btnReturn_Click(object sender, EventArgs e)
    {
        ...
        lbLablename.Visible = false;
        txtLabel.Visible = false;
        btnSaveLable.Visible = false;
    ...
    protected void rptLable_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        Users user = (Users)Session["user"];
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
            DataRowView drvw = (DataRowView)e.Item.DataItem;
            string lablename = drvw["lablename"].ToString();
            string sql = "select * from Users where userid in (select friendid from Lable where userid='"+user.Userid+"' and lablename=N'"+lablename+"')";
            DataTable dt = new DataTable();
            dt = DataClass.DataT(sql);
            Repeater rept = (Repeater)e.Item.FindControl("rptLableFriend");
            // rept = (Repeater)e.Item.FindControl("rptLableFriend");
            rept.DataSource = dt;
            rept.DataBind();
        }
    }
